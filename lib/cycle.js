module.exports = (function() {
  ...
  Cycle.prototype.tick = function() {
    ...
    var collidedPath = null;
    if (collidedPath = this._newPositionCollidedPath()) {
      if (collidedPath.cycle.player.playerId !== this.player.playerId) {
        collidedPath.cycle.player.score += 1;
      }
      this._die();
    } else if (this._newPositionCollidesWithBoundary()) {
      this._die();
    }
  }
  ...
  // Returns the path that this cycle is currently colliding with. If no collision, returns false.
  Cycle.prototype._newPositionCollidedPath = function() {
