module.exports = (function() {
  ...
  Game.prototype.serialize = function() {
    var state = {
      cycles: this.players.map(function(player) { return player.cycle.serialize(); })
    }
  ...
  Game.prototype._loop = function() {
    this.grid.paths = this.players.map(function(player) { return player.cycle.path });
  ...
  Game.prototype._createCycle = function(playerId) {
    for (var i = 0; i < this.players.length; i++) {
      if (this.players[i].playerId === playerId) {
        this.players[i].cycle = new Cycle(playerId, this.grid);
      }
    }
  }
